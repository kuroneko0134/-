...
`web/app.html` は、全譜面をタップで埋めていく記録アプリです。Artifact として公開すると、スマホのブラウザからそのまま使えます。

- 全 719 曲 / 3726 譜面をページ内に同梱しているため、開いた時点で通信なしに動きます。
- 行をタップするたびに 未クリア → クリア → フルコンボ → AP と切り替わります。
- 難易度タブ、レベル絞り込み、曲名検索、「未達成のみ」表示、レベルごとの一括クリアがあります。
- 記録は端末内に保存し、同期が使える環境では端末をまたいで共有されます。同じ譜面の記録は良い方が残ります。
- CSV で書き出せば、そのまま `proseka me import` に渡せます。

同梱データは次のコマンドで再生成します。

```bash
python3 tools/build_web_data.py            # web/data/charts.json を更新
python3 tools/build_web_data.py --region en
```
